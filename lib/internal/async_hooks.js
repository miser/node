 ...
 *    current execution stack. // async_id 分配给 负责当前执行栈 的资源
 ...
 *  // resource的async_id 是 触发对应当前执行栈 的资源
 *  kAsyncIdCounter: Incremental counter tracking the next assigned async_id.
 *  // 累计计数器，跟踪下个被分配的 async_id
 ...
 *    // 在一个资源的constructor 设置 init() 的 triggerAsyncId 被立即写入
 *    // 有限排序检索 triggerAsyncId 
 *    1. the value passed directly to the constructor // 直接传给 constructor 的值
 *    2. value set in kDefaultTriggerAsyncId // 在 kDefaultTriggerAsyncId 设置的值
 *    3. executionAsyncId of the current resource. // 当前资源的executionAsyncId
 *
...
function emitInitNative(asyncId, type, triggerAsyncId, resource) {
  active_hooks.call_depth += 1; // 0 表示最外层
