...
<<<<<<< HEAD
  if (Array.isArray(options.args)) this.spawnargs = options.args;
  else if (options.args === undefined) this.spawnargs = [];
  else throw new ERR_INVALID_ARG_TYPE("options.args", "Array", options.args);
=======
...
>>>>>>> aa1eb1fd597f68c6cb67c2a4d631a8d66f2cb94e
  ...
  if (
    ...
    if (err === UV_EMFILE || err === UV_ENFILE) return err;
  } else if (err) {
...
ChildProcess.prototype.kill = function (sig) {
  const signal =
    sig === 0 ? sig : convertToValidSignal(sig === undefined ? "SIGTERM" : sig);
=======
...
const channelDeprecationMsg =
  "_channel is deprecated. " + "Use ChildProcess.channel instead.";
...
>>>>>>> aa1eb1fd597f68c6cb67c2a4d631a8d66f2cb94e
    ...
    if (
      ...
      if (target._pendingMessage) {
        if (message.cmd === "NODE_HANDLE_ACK") {
          ...
          target._pendingMessage.retransmissions++ ===
  ...
  target._send = function (message, handle, options, callback) {
    ...
    if (handle) {
      ...
      handle = handleConversion[message.type].send.call(
        target,
        message,
        handle,
        options
