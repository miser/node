...
function createWorkerProcess(id, env) {
  ...
  if (
    ...
    if ("inspectPort" in cluster.settings) {
      ...
      else inspectPort = cluster.settings.inspectPort;
...
cluster.fork = function (env) {
  ...
  cluster.workers[worker.id] = worker; // 键值对 存储 worker
  return worker;
};
...
<<<<<<< HEAD
  // address port addressType fd 相同的情况下， 当前运行时的 index 会不断增加 可以区分不同的 子进程
  const key =
